...
| F6 Reputación | 🟡 | CRDT + gossip OK. Los `JobSuccess/JobFailure` se aplican al **propio peer_id** (autograding), no a la contraparte, y los eventos no se firman. |
| F7 Trust Graph + Validación | 🟡 | Aggregación + transitividad de 2.º orden OK, pero **la firma del `TrustEdge` se acepta sin verificar**. Edges falsificadas no se rechazan. |
...
8. **Reputación auto-emitida.** El handler de jobs llama
   `record_job_outcome` sobre el `peer_id` local, no sobre el del
   solicitante. Los eventos viajan por gossip sin firma, así que un
   nodo puede inflar su propio score.
9. **TrustGraph sin verificación.** `add_edge` no verifica
   `edge.signature`. Cualquier nodo puede publicar `TrustEdge` que
   afirme "X confía en Y al 100%".
