...
impl S3Backend {
    ...
    pub fn new(
        ...
        let auth = base64::engine::general_purpose::STANDARD
            .encode(format!("{access_key}:{secret_key}"));
    ...
    async fn s3_request(
        ...
        let ct_owned = content_type.unwrap_or("application/octet-stream").to_string();
        ...
        tokio::task::spawn_blocking(move || -> Result<Vec<u8>, StorageError> {
            ...
            let req_builder = match method_owned.as_str() {
                ...
                _ => {
                    return Err(StorageError::Backend(format!("unsupported method: {method_owned}")));
                }
            };
            ...
            if status.is_success() || status == 404 {
                ...
                Err(StorageError::Backend(format!("S3 {method_owned} {url_owned}: {status} - {msg}")))
            }
        })
        .await
...
impl StorageBackend for S3Backend {
    async fn add(&self, bytes: &[u8], opts: StorageOpts) -> Result<CID, StorageError> {
        ...
        self.s3_request("PUT", &url, Some(bytes.to_vec()), None).await?;
    ...
    async fn pin(&self, cid: &CID, policy: PinPolicy) -> Result<(), StorageError> {
        let mut pins = self.pins.lock().map_err(|e| {
    ...
    async fn ls_pins(&self) -> Result<Vec<PinnedItem>, StorageError> {
        let pins = self.pins.lock().map_err(|e| {
            StorageError::Backend(format!("lock error: {e}"))
        })?;
